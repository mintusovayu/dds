...
- **Точность float через ``pytest.approx``.** Нормализованные
  ...
  идёт с допуском.
- **Совместимость с текущим ``WordEntry``.** Модель ``WordEntry``
  в Фазе 0 не содержит поля ``block_no`` (оно добавляется в
  Фазе 3 вместе с упрощением ``build_word_index``). Хелперы ниже
  конструируют ``WordEntry`` только с полями, существующими на
  данный момент. Ключ ``by_line`` остаётся кортежем
  ``(block_no, line_no)`` — это независимая структура словаря,
  а не поле модели.
...
def _make_entry(
    *,
    ...
    Поля ``block_no`` в модели нет — оно появится в Фазе 3 вместе
    с упрощением ``build_word_index``. Ключи словаря ``by_line``
    формируются независимо от модели (см. хелперы ниже), поэтому
    отсутствие поля не влияет на тесты.
